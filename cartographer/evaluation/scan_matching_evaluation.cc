...
Sample GenerateDefinedSampleSlice(double error_x, double error_y, double error_phi,
                             const ScanCloudGenerator::ModelType model_type,
                             const Eigen::Vector3d size,
                             const double resolution,
                             float cloud_noise_std_dev,
                             float azimuth, float fov) {
  cartographer::sensor::RangeData range_data;
...
void RunScanMatchingEvaluation() {
  ...
  auto parameter_dictionary_range_data_inserter = common::MakeDictionary(R"text(
      ...
      tsdf_range_data_inserter = {
        relative_truncation_distance = 3,
        maximum_weight = 1000.,
  ...
  Sample sample = GenerateDefinedSample(0.0, 0.0, 0.0,
                                        ...
                                        {2.25, 1.25, 1.25}, 0.031, 0.f);
  //  sample.range_data.returns.clear();
  //
  //  sensor::RangefinderPoint point_t;
  //  point_t.position = {-1.f, -0.11f, 0.0001f};
  //  sensor::PointCloud single_point_t = {point_t};
...
void RunScanMatchingEvaluationRotatingScan() {
  ...
  LOG(INFO) << "wrote file";
    ...
    for (double phi = -M_PI; phi < 5 * M_PI; phi += 0.1 * M_PI) {
      ...
      Sample sample = GenerateDefinedSampleSlice(0.0, 0.0, 0.0,
                                            ScanCloudGenerator::ModelType::CUBOID,
...
int main(int argc, char** argv) {
  ...
  cartographer::evaluation::RunScanMatchingEvaluationDriftTest();
}