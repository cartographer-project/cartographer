...
constexpr std::size_t kNumberOfCells = 32768;
...
float SlowValueToBoundedFloat(const uint16 value, const uint16 unknown_value,
  ...
  CHECK_LT(value, kNumberOfCells);
  if (value == unknown_value) return unknown_result;
  const float kScale = (upper_bound - lower_bound) / (kNumberOfCells - 2.f);
...
std::unique_ptr<std::vector<float>> PrecomputeValueToBoundedFloat(
  ...
  for (int repeat = 0; repeat != 2; ++repeat) {
    for (int value = 0; value != kNumberOfCells; ++value) {
...
std::vector<uint16> ComputeLookupTableToApplyOdds(const float odds) {
  ...
  result.push_back(ProbabilityToValue(ProbabilityFromOdds(odds)) +
                   kUpdateMarker);
  for (int cell = 1; cell != kNumberOfCells; ++cell) {
