...
float Grid2D::GetCorrespondenceCost(const Eigen::Array2i& cell_index) const {
  if (limits_.Contains(cell_index)) {
    return ValueToProbability(correspondence_cost_cells_[ToFlatIndex(cell_index, limits_)]); //todo(kdaun) Create ValueToCorrespondence
  }
...
bool Grid2D::IsKnown(const Eigen::Array2i& cell_index) const {
  return limits_.Contains(cell_index) &&
      correspondence_cost_cells_[ToFlatIndex(cell_index, limits_)] != kUnknownProbabilityValue;
}
...
proto::Grid2D Grid2D::ToProto() const {
  ...
  *result.mutable_limits() = mapping::ToProto(limits_);
  result.mutable_correspondence_cost_cells()->Reserve(correspondence_cost_cells_.size());
