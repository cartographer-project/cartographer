...
using NodeOptInput = NodeOptInput3D;
using SubmapOptInput = SubmapOptInput3D;
...
transform::Rigid3d GetInitialLandmarkPose(
    ...
    const NodeOptInput& prev_node, const NodeOptInput& next_node,
...
void AddLandmarkCostFunctions(
    ...
    bool freeze_landmarks, const MapById<NodeId, NodeOptInput>& node_data,
...
void OptimizationProblem3D::InsertTrajectoryNode(
    const NodeId& node_id, const NodeOptInput& node_data) {
...
void OptimizationProblem3D::AddSubmap(
  ...
  submap_data_.Append(trajectory_id, SubmapOptInput{global_submap_pose});
}

void OptimizationProblem3D::InsertSubmap(
    const SubmapId& submap_id, const transform::Rigid3d& global_submap_pose) {
  submap_data_.Insert(submap_id, SubmapOptInput{global_submap_pose});
}
...
void OptimizationProblem3D::Solve(
  ...
  if (!options_.fix_z_in_3d()) {
    for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
      ...
      for (++node_it; node_it != trajectory_end; ++node_it) {
        ...
        const NodeOptInput& first_node_data = prev_node_it->data;
        prev_node_it = node_it;
        const NodeId second_node_id = node_it->id;
        const NodeOptInput& second_node_data = node_it->data;
        ...
        if (next_node_it != trajectory_end &&
          ...
          const NodeOptInput& third_node_data = next_node_it->data;
  ...
  for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
    ...
    for (; node_it != trajectory_end; ++node_it) {
      ...
      const NodeOptInput& node_data = node_it->data;
...
OptimizationProblem3D::CalculateOdometryBetweenNodes(
    const int trajectory_id, const NodeOptInput& first_node_data,
    const NodeOptInput& second_node_data) const {
