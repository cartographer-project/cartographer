...
struct NodeOptInput3D {
  common::Time time;
  transform::Rigid3d local_pose;
  transform::Rigid3d global_pose;
};

struct SubmapOptInput3D {
  transform::Rigid3d global_pose;
};

class OptimizationProblem3D
    : public OptimizationProblemInterface<NodeOptInput3D, SubmapOptInput3D,
                                          transform::Rigid3d> {
 public:
  ...
  void AddTrajectoryNode(int trajectory_id,
                         const NodeOptInput3D& node_data) override;
  ...
  const MapById<NodeId, NodeOptInput3D>& node_data() const override {
    return node_data_;
  }
  const MapById<SubmapId, SubmapOptInput3D>& submap_data() const override {
    return submap_data_;
  }
 ...
 private:
  ...
  std::unique_ptr<transform::Rigid3d> CalculateOdometryBetweenNodes(
      int trajectory_id, const NodeOptInput3D& first_node_data,
      const NodeOptInput3D& second_node_data) const;

  pose_graph::proto::OptimizationProblemOptions options_;
  MapById<NodeId, NodeOptInput3D> node_data_;
  MapById<SubmapId, SubmapOptInput3D> submap_data_;
