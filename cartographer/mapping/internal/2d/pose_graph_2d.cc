...
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
  ...
  if (insertion_submaps.size() == 1) {
    ...
    if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0) {
      if (model_.initial_trajectory_poses.count(trajectory_id) > 0) {
        model_.trajectory_connectivity_state.Connect(
            trajectory_id,
            model_.initial_trajectory_poses.at(trajectory_id).to_trajectory_id,
            time);
      }
      optimization_problem_->AddSubmap(
          trajectory_id, transform::Project2D(
                             ComputeLocalToGlobalTransform(
                                 model_.global_submap_poses_2d, trajectory_id) *
                             insertion_submaps[0]->local_pose()));
    }
    CHECK_EQ(1, submap_data.SizeOfTrajectoryOrZero(trajectory_id));
    const SubmapId submap_id{trajectory_id, 0};
    CHECK(model_.submap_data.at(submap_id).submap == insertion_submaps.front());
    return {submap_id};
  }
  ...
  if (model_.submap_data.at(last_submap_id).submap ==
  ...
  CHECK(model_.submap_data.at(last_submap_id).submap ==
        insertion_submaps.back());
  const SubmapId front_submap_id{trajectory_id,
                                 last_submap_id.submap_index - 1};
  CHECK(model_.submap_data.at(front_submap_id).submap ==
        insertion_submaps.front());
...
NodeId PoseGraph2D::AddNode(
  ...
  const NodeId node_id = model_.trajectory_nodes.Append(
      trajectory_id, TrajectoryNode{constant_data, optimized_pose});
  ++model_.num_trajectory_nodes;

  // Test if the 'insertion_submap.back()' is one we never saw before.
  if (model_.submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0 ||
      std::prev(model_.submap_data.EndOfTrajectory(trajectory_id))
              ->data.submap != insertion_submaps.back()) {
    // We grow 'model_.submap_data' as needed. This code assumes that the first
    // time we see a new submap is as 'insertion_submaps.back()'.
    const SubmapId submap_id =
        model_.submap_data.Append(trajectory_id, InternalSubmapData());
    model_.submap_data.at(submap_id).submap = insertion_submaps.back();
  }
...
void PoseGraph2D::AddTrajectoryIfNeeded(const int trajectory_id) {
  model_.trajectory_connectivity_state.Add(trajectory_id);
...
void PoseGraph2D::AddLandmarkData(int trajectory_id,
  ...
  AddWorkItem([=]() REQUIRES(mutex_) {
    for (const auto& observation : landmark_data.landmark_observations) {
      model_.landmark_nodes[observation.id].landmark_observations.emplace_back(
...
void PoseGraph2D::ComputeConstraint(const NodeId& node_id,
                                    const SubmapId& submap_id) {
  CHECK(model_.submap_data.at(submap_id).state == SubmapState::kFinished);

  const common::Time node_time = GetLatestNodeTime(node_id, submap_id);
  const common::Time last_connection_time =
      model_.trajectory_connectivity_state.LastConnectionTime(
  ...
  if (node_id.trajectory_id == submap_id.trajectory_id ||
    ...
    constraint_builder_.MaybeAddConstraint(
        ...
        static_cast<const Submap2D*>(
            model_.submap_data.at(submap_id).submap.get()),
        node_id, model_.trajectory_nodes.at(node_id).constant_data.get(),
...
void PoseGraph2D::ComputeConstraintsForOldNodes(const SubmapId& submap_id) {
  const auto& submap_data = model_.submap_data.at(submap_id);
...
void PoseGraph2D::ComputeConstraintsForNode(
  ...
  const auto& constant_data = model_.trajectory_nodes.at(node_id).constant_data;
  ...
  for (size_t i = 0; i < insertion_submaps.size(); ++i) {
    ...
    // only be marked as finished in 'model_.submap_data' further below.
    CHECK(model_.submap_data.at(submap_id).state == SubmapState::kActive);
    model_.submap_data.at(submap_id).node_ids.emplace(node_id);
    const transform::Rigid2d constraint_transform =
        constraints::ComputeSubmapPose(*insertion_submaps[i]).inverse() *
        local_pose_2d;
    model_.constraints.push_back(
  ...
  for (const auto& submap_id_data : model_.submap_data) {
  ...
  if (newly_finished_submap) {
    ...
    InternalSubmapData& finished_submap_data =
        model_.submap_data.at(finished_submap_id);
...
common::Time PoseGraph2D::GetLatestNodeTime(const NodeId& node_id,
  ...
  common::Time time = model_.trajectory_nodes.at(node_id).constant_data->time;
  const InternalSubmapData& submap_data = model_.submap_data.at(submap_id);
  if (!submap_data.node_ids.empty()) {
    const NodeId last_submap_node_id =
        *model_.submap_data.at(submap_id).node_ids.rbegin();
    time = std::max(
        time,
        model_.trajectory_nodes.at(last_submap_node_id).constant_data->time);
  }
  return time;
}
...
void PoseGraph2D::HandleWorkQueue(
  ...
  {
    common::MutexLocker locker(&mutex_);
    model_.constraints.insert(model_.constraints.end(), result.begin(),
                              result.end());
  }
  RunOptimization();
...
void PoseGraph2D::WaitForAllComputations() {
  ...
  while (!locker.AwaitWithTimeout(
    ...
    if (model_.num_trajectory_nodes != num_finished_nodes_at_start) {
...
void PoseGraph2D::FinishTrajectory(const int trajectory_id) {
  ...
  AddWorkItem([this, trajectory_id]() REQUIRES(mutex_) {
    CHECK_EQ(model_.finished_trajectories.count(trajectory_id), 0);
    model_.finished_trajectories.insert(trajectory_id);

    for (const auto& submap : model_.submap_data.trajectory(trajectory_id)) {
      model_.submap_data.at(submap.id).state = SubmapState::kFinished;
    }
    CHECK(!run_loop_closure_);
    DispatchOptimization();
  });
...
bool PoseGraph2D::IsTrajectoryFinished(const int trajectory_id) const {
  return model_.finished_trajectories.count(trajectory_id) > 0;
}
...
void PoseGraph2D::FreezeTrajectory(const int trajectory_id) {
  ...
  model_.trajectory_connectivity_state.Add(trajectory_id);
  AddWorkItem([this, trajectory_id]() REQUIRES(mutex_) {
    CHECK_EQ(model_.frozen_trajectories.count(trajectory_id), 0);
    model_.frozen_trajectories.insert(trajectory_id);
  });
}

bool PoseGraph2D::IsTrajectoryFrozen(const int trajectory_id) const {
  return model_.frozen_trajectories.count(trajectory_id) > 0;
}
...
void PoseGraph2D::AddSubmapFromProto(
  ...
  model_.submap_data.Insert(submap_id, InternalSubmapData());
  model_.submap_data.at(submap_id).submap = submap_ptr;
  // Immediately show the submap at the 'global_submap_pose'.
  model_.global_submap_poses_2d.Insert(
  ...
  AddWorkItem([this, submap_id, global_submap_pose_2d]() REQUIRES(mutex_) {
    model_.submap_data.at(submap_id).state = SubmapState::kFinished;
...
void PoseGraph2D::AddNodeToSubmap(const NodeId& node_id,
  ...
  AddWorkItem([this, node_id, submap_id]() REQUIRES(mutex_) {
    model_.submap_data.at(submap_id).node_ids.insert(node_id);
  });
}
...
void PoseGraph2D::AddSerializedConstraints(
  ...
  AddWorkItem([this, constraints]() REQUIRES(mutex_) {
    for (const auto& constraint : constraints) {
      CHECK(model_.trajectory_nodes.Contains(constraint.node_id));
      CHECK(model_.submap_data.Contains(constraint.submap_id));
      CHECK(model_.trajectory_nodes.at(constraint.node_id).constant_data !=
            nullptr);
      CHECK(model_.submap_data.at(constraint.submap_id).submap != nullptr);
      switch (constraint.tag) {
        case Constraint::Tag::INTRA_SUBMAP:
          CHECK(model_.submap_data.at(constraint.submap_id)
      ...
      const Constraint::Pose pose = {
          constraint.pose.zbar_ij *
              transform::Rigid3d::Rotation(
                  model_.trajectory_nodes.at(constraint.node_id)
                      .constant_data->gravity_alignment.inverse()),
...
void PoseGraph2D::RunOptimization() {
  ...
  // model_.constraints, model_.frozen_trajectories and model_.landmark_nodes
  ...
  optimization_problem_->Solve(model_.constraints, model_.frozen_trajectories,
                               model_.landmark_nodes);
  common::MutexLocker locker(&mutex_);
  ...
  for (const int trajectory_id : node_data.trajectory_ids()) {
    for (const auto& node : node_data.trajectory(trajectory_id)) {
      auto& mutable_trajectory_node = model_.trajectory_nodes.at(node.id);
    ...
    const NodeId last_optimized_node_id =
        std::prev(node_data.EndOfTrajectory(trajectory_id))->id;
    auto node_it =
        std::next(model_.trajectory_nodes.find(last_optimized_node_id));
    for (; node_it != model_.trajectory_nodes.EndOfTrajectory(trajectory_id);
         ++node_it) {
      auto& mutable_trajectory_node = model_.trajectory_nodes.at(node_it->id);
      mutable_trajectory_node.global_pose =
          old_global_to_new_global * mutable_trajectory_node.global_pose;
    }
  }
  for (const auto& landmark : optimization_problem_->landmark_data()) {
    model_.landmark_nodes[landmark.first].global_landmark_pose =
        landmark.second;
  }
  model_.global_submap_poses_2d = submap_data;
}

MapById<NodeId, TrajectoryNode> PoseGraph2D::GetTrajectoryNodes() const {
  common::MutexLocker locker(&mutex_);
  return model_.trajectory_nodes;
}
...
MapById<NodeId, TrajectoryNodePose> PoseGraph2D::GetTrajectoryNodePoses()
  ...
  common::MutexLocker locker(&mutex_);
  for (const auto& node_id_data : model_.trajectory_nodes) {
...
std::map<std::string, transform::Rigid3d> PoseGraph2D::GetLandmarkPoses()
  ...
  common::MutexLocker locker(&mutex_);
  for (const auto& landmark : model_.landmark_nodes) {
...
void PoseGraph2D::SetLandmarkPose(const std::string& landmark_id,
  ...
  common::MutexLocker locker(&mutex_);
  AddWorkItem([=]() REQUIRES(mutex_) {
    model_.landmark_nodes[landmark_id].global_landmark_pose = global_pose;
  });
}
...
void PoseGraph2D::SetInitialTrajectoryPose(const int from_trajectory_id,
                                           ...
                                           const common::Time time) {
  common::MutexLocker locker(&mutex_);
  model_.initial_trajectory_poses[from_trajectory_id] =
      InitialTrajectoryPose{to_trajectory_id, pose, time};
}

transform::Rigid3d PoseGraph2D::GetInterpolatedGlobalTrajectoryPose(
    const int trajectory_id, const common::Time time) const {
  CHECK_GT(model_.trajectory_nodes.SizeOfTrajectoryOrZero(trajectory_id), 0);
  const auto it = model_.trajectory_nodes.lower_bound(trajectory_id, time);
  if (it == model_.trajectory_nodes.BeginOfTrajectory(trajectory_id)) {
    return model_.trajectory_nodes.BeginOfTrajectory(trajectory_id)
        ->data.global_pose;
  }
  if (it == model_.trajectory_nodes.EndOfTrajectory(trajectory_id)) {
    return std::prev(model_.trajectory_nodes.EndOfTrajectory(trajectory_id))
        ->data.global_pose;
  }
...
transform::Rigid3d PoseGraph2D::GetLocalToGlobalTransform(
  ...
  common::MutexLocker locker(&mutex_);
  return ComputeLocalToGlobalTransform(model_.global_submap_poses_2d,
                                       trajectory_id);
}

std::vector<std::vector<int>> PoseGraph2D::GetConnectedTrajectories() const {
  return model_.trajectory_connectivity_state.Components();
}
...
transform::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(
  ...
  if (begin_it == end_it) {
    const auto it = model_.initial_trajectory_poses.find(trajectory_id);
    if (it != model_.initial_trajectory_poses.end()) {
  ...
  return transform::Embed3D(
         ...
         model_.submap_data.at(last_optimized_submap_id)
             .submap->local_pose()
             .inverse();
}
...
PoseGraphInterface::SubmapData PoseGraph2D::GetSubmapDataUnderLock(
    const SubmapId& submap_id) const {
  const auto it = model_.submap_data.find(submap_id);
  if (it == model_.submap_data.end()) {
    return {};
  }
  auto submap = it->data.submap;
  if (model_.global_submap_poses_2d.Contains(submap_id)) {
    // We already have an optimized pose.
    return {submap,
            transform::Embed3D(
                model_.global_submap_poses_2d.at(submap_id).global_pose)};
  }
  // We have to extrapolate.
  return {submap, ComputeLocalToGlobalTransform(model_.global_submap_poses_2d,
...
PoseGraph2D::TrimmingHandle::GetOptimizedSubmapData() const {
  ...
  for (const auto& submap_id_data : parent_->model_.submap_data) {
    if (submap_id_data.data.state != SubmapState::kFinished ||
        !parent_->model_.global_submap_poses_2d.Contains(submap_id_data.id)) {
      continue;
    }
    submaps.Insert(
        submap_id_data.id,
        SubmapData{submap_id_data.data.submap,
                   transform::Embed3D(parent_->model_.global_submap_poses_2d
...
void PoseGraph2D::TrimmingHandle::MarkSubmapAsTrimmed(
  ...
  CHECK(parent_->model_.submap_data.at(submap_id).state ==
  ...
  for (const Constraint& constraint : parent_->model_.constraints) {
    if (constraint.tag == Constraint::Tag::INTRA_SUBMAP &&
        constraint.submap_id != submap_id) {
      nodes_to_retain.insert(constraint.node_id);
    }
  }
  // Remove all 'model_.constraints' related to 'submap_id'.
  std::set<NodeId> nodes_to_remove;
  {
    ...
    parent_->model_.constraints = std::move(constraints);
  }
  // Remove all 'model_.constraints' related to 'nodes_to_remove'.
  {
    std::vector<Constraint> constraints;
    for (const Constraint& constraint : parent_->model_.constraints) {
  ...
  CHECK(parent_->model_.submap_data.at(submap_id).state ==
        SubmapState::kFinished);
  parent_->model_.submap_data.Trim(submap_id);
  ...
  for (const NodeId& node_id : nodes_to_remove) {
    parent_->model_.trajectory_nodes.Trim(node_id);
