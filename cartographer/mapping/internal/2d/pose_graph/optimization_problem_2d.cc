...
void OptimizationProblem2D::Solve(
  ...
  for (const auto& node_id_data : node_data_) {
    ...
    C_nodes.Insert(node_id_data.id, FromPose(node_id_data.data.global_2d_pose));
  ...
  for (auto node_it = node_data_.begin(); node_it != node_data_.end();) {
    ...
    for (++node_it; node_it != trajectory_end; ++node_it) {
      ...
      const transform::Rigid3d relative_local_slam_pose =
          transform::Embed3D(first_node_data.local_2d_pose.inverse() *
                             second_node_data.local_2d_pose);
      problem.AddResidualBlock(
  ...
  for (const auto& C_node_id_data : C_nodes) {
    node_data_.at(C_node_id_data.id).global_2d_pose =
