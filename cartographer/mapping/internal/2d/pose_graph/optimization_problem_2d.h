...
struct SubmapOptInput2D {
  transform::Rigid2d global_pose;
};

class OptimizationProblem2D
    : public OptimizationProblemInterface<NodeOptInput2D, SubmapOptInput2D,
                                          transform::Rigid2d> {
 public:
  ...
  void AddTrajectoryNode(int trajectory_id,
                         const NodeOptInput2D& node_data) override;
  ...
  const MapById<NodeId, NodeOptInput2D>& node_data() const override {
    return node_data_;
  }
  const MapById<SubmapId, SubmapOptInput2D>& submap_data() const override {
    return submap_data_;
  }
 ...
 private:
  ...
  std::unique_ptr<transform::Rigid3d> CalculateOdometryBetweenNodes(
      int trajectory_id, const NodeOptInput2D& first_node_data,
      const NodeOptInput2D& second_node_data) const;

  pose_graph::proto::OptimizationProblemOptions options_;
  MapById<NodeId, NodeOptInput2D> node_data_;
  MapById<SubmapId, SubmapOptInput2D> submap_data_;
