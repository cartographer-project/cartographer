...
TEST(VoxelFilterTest, ReturnsTheFirstPointInEachVoxel) {
  PointCloud point_cloud = {
      Eigen::Vector3f{0.f, 0.f, 0.f}, Eigen::Vector3f{0.1f, -0.1f, 0.1f},
      Eigen::Vector3f{0.3f, -0.1f, 0.f}, Eigen::Vector3f{0.f, 0.f, 0.1f}};
...
TEST(VoxelFilterTest, IgnoresTime) {
  ...
  for (int i = 0; i < 100; ++i) {
    timed_point_cloud.emplace_back(Eigen::Vector3f{-100.f, 0.3f, 0.4f},
                                   1.f * i);
  }
