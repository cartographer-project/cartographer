...
TEST(LaserTest, ToLaserFan) {
  ...
  const LaserFan fan = ToLaserFan(laser_scan, 0.f, 10.f);
  ...
  EXPECT_TRUE(fan.misses.empty());
  EXPECT_TRUE(fan.reflectivities.empty());
}

TEST(LaserTest, ToLaserFanWithInfinityAndNaN) {
  proto::LaserScan laser_scan;
  laser_scan.add_range()->add_value(1.f);
  ...
  const LaserFan fan = ToLaserFan(laser_scan, 2.f, 10.f);
