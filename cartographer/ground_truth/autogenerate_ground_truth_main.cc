...
DEFINE_double(min_covered_distance, 100.,
              "Minimum distance covered before a loop closure is considered a "
              "candidate for autogenerated ground truth.");
...
// We pick the representative in the middle of the submap.
//
...
std::vector<int> ComputeSubmapRepresentative(
...
proto::GroundTruth GenerateGroundTruth(
  ...
  const std::vector<int> submap_to_scan_index =
      ComputeSubmapRepresentative(pose_graph);
  ...
  for (const auto& constraint : pose_graph.constraint()) {
    ...
    // A representative node for the submap must have been chosen, i.e. not be
    // the very end, but those should not be part of loop closure anyway.
    CHECK_EQ(constraint.submap_id().trajectory_id(), 0);
    CHECK_EQ(constraint.scan_id().trajectory_id(), 0);
