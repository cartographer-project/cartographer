...
TEST(MapLimitsTest, ToProto) {
  ...
  EXPECT_EQ(limits.max().x(), proto.max_x());
  EXPECT_EQ(limits.max().y(), proto.max_y());
...
TEST(MapLimitsTest, ProtoConstructor) {
  ...
  limits.set_max_x(2.0);
  limits.set_max_y(3.0);
  ...
  EXPECT_EQ(limits.max_x(), native.max().x());
  EXPECT_EQ(limits.max_y(), native.max().y());
