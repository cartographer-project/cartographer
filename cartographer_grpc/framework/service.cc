...
void Service::HandleEvent(Rpc::Event event, Rpc* rpc, bool ok) {
  rpc->UpdateRpcEventState(event, false);
  switch (event) {
...
void Service::RemoveIfNotPending(Rpc* rpc) {
  if (!rpc->IsRpcEventPending(Rpc::Event::DONE) &&
      !rpc->IsRpcEventPending(Rpc::Event::READ) &&
      !rpc->IsRpcEventPending(Rpc::Event::WRITE) &&
      !rpc->IsRpcEventPending(Rpc::Event::FINISH)) {
    active_rpcs_.Remove(rpc);
  }
}
